...
-- RLS: only owner (via profile.user_id) can manage their rows
create policy if not exists "Users can view their own always_never"
...
create policy if not exists "Users can insert their own always_never"
  on public.always_never for insert
  with check (exists (
    select 1 from public.profiles p where p.id = always_never.profile_id and p.user_id = auth.uid()
  ));

create policy if not exists "Users can update their own always_never"
  on public.always_never for update
  using (exists (
    select 1 from public.profiles p where p.id = always_never.profile_id and p.user_id = auth.uid()
  ));

create policy if not exists "Users can delete their own always_never"
  on public.always_never for delete
  using (exists (
    select 1 from public.profiles p where p.id = always_never.profile_id and p.user_id = auth.uid()
  ));

create policy if not exists "Users can view their own agree_disagree"
...
create policy if not exists "Users can insert their own agree_disagree"
  on public.agree_disagree for insert
  with check (exists (
    select 1 from public.profiles p where p.id = agree_disagree.profile_id and p.user_id = auth.uid()
  ));

create policy if not exists "Users can update their own agree_disagree"
  on public.agree_disagree for update
  using (exists (
    select 1 from public.profiles p where p.id = agree_disagree.profile_id and p.user_id = auth.uid()
  ));

create policy if not exists "Users can delete their own agree_disagree"
...
-- Timestamp triggers
create trigger if not exists update_always_never_updated_at
  before update on public.always_never
  for each row execute function public.update_updated_at_column();

create trigger if not exists update_agree_disagree_updated_at
